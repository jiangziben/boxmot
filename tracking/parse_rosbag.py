...
import argparse
import os
from tracking.msg import FloatArray3
def main():
    # 创建解析器
    parser = argparse.ArgumentParser(description='参数解析示例')

    # 添加参数
    parser.add_argument('-i', '--input_file', type=str,default="/home/jiangziben/data/people_tracking/3d/follow.bag", help='输入文件路径')
    parser.add_argument('-o', '--output_folder', type=str,default="/home/jiangziben/data/people_tracking/3d/follow",help='输出文件夹路径')
    ...
    # 解析 rosbag 文件中的图像信息
    for topic, msg, t in bag.read_messages(topics=[image_topic, depth_image_topic]):
