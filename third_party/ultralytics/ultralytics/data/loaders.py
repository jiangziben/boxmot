...
class LoadImagesAndVideos:
    ...
    def __init__(self, path, batch=1, vid_stride=1):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            ...
            if "*" in a:
                ...
                if (Path(a)/"Color").exists() and (Path(a)/"Depth").exists():
                    files.extend(sorted(glob.glob(os.path.join(a, "Color", "*.*"))))  # dir
                    depth_files.extend(sorted(glob.glob(os.path.join(a, "Depth", "*.*"))))  # dir
                else:
    ...
    def __next__(self):
        ...
        paths, imgs, info, depths = [], [], [],[]
        while len(imgs) < self.bs:
            if self.count >= self.nf:  # end of file list
                if imgs:
                    return paths, imgs, info, depths  # return last partial batch
                else:
                    raise StopIteration
            ...
            if self.video_flag[self.count]:
                ...
                if path.split(".")[-1].lower() == "heic":
                    ...
                    im0 = imread(path)  # BGR
                    depth0 = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)
                if im0 is None:
                    LOGGER.warning(f"WARNING ⚠️ Image Read Error {path}")
                elif depth0 is None:
                    LOGGER.warning(f"WARNING ⚠️ Depth Read Error {depth_path}")
                else:
                    paths.append(path)
                    imgs.append(im0)
